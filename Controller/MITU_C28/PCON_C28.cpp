...
void main(void)
{
   ...
   //GpioG1DataRegs.GPADAT.bit.GPIO7 = 0; //Remove system OK flag to FPGA
   ...
   Init_SPI(); //Setup SPI pins
    ...
    GpioG1DataRegs.GPADAT.bit.GPIO2 = 0;
   	GpioG1DataRegs.GPADAT.bit.GPIO3 = 0;
   	GpioG1DataRegs.GPADAT.bit.GPIO4 = 0;
   	GpioG1DataRegs.GPADAT.bit.GPIO5 = 0;
   	GpioG1DataRegs.GPADAT.bit.GPIO6 = 0;
   	GpioG1DataRegs.GPADAT.bit.GPIO7 = 0;
   ...
   //Setup_DMA(MEASURMENTS_BUFFER_SIZE_HALF);
    ...
    FlashReader = new SPIFlashReader();
...
// Step. Main loop.
    for(;;)
    {
    	...
    	WatchdogValue=1; GpioG1DataRegs.GPASET.bit.GPIO5 = 1; // Watchdog out

    	LedCounter ++;   if (LedCounter == 500)  {GpioG1DataRegs.GPACLEAR.bit.GPIO10 = 1;} // LED Blink // Turn off LED
    	if (LedCounter > 1000) 	{GpioG1DataRegs.GPASET.bit.GPIO10 = 1; LedCounter = 0;} // Turn on LED
    	// ----------------------------------------------------------------------
    	...
    	WatchdogValue=0; GpioG1DataRegs.GPACLEAR.bit.GPIO5 = 1; // Watchdog out
    	...
    	while (CpuTimer1Regs.TCR.bit.TIF==0)
    	{
    		...
    		CPU_OverloadFlag=0;
    		WaitCyclesCounter++;
    		if (WaitCyclesCounter>MAIN_CYCLE_US) {ErrorSet(ERR_MAIN_IRQ_LOSS);}
    	}
