...
//		taskParams.stackSize = 1024;
		...
		Task_Handle taskHandle = Task_create(
			(Task_FuncPtr)OsciConnectionHandler,
			&taskParams,
			&eb
		);
		if (taskHandle == NULL) {
...
int tcp_send_all(SOCKET fd, volatile const void *buf, size_t len) {
	if (len == 0) {
		return 0;
	}
	size_t bytes_sent = 0;
...
Void OsciConnectionHandler(UArg arg0, UArg arg1) {
	...
	opt = 1;
	status = setsockopt(client_fd, SOL_SOCKET, SO_KEEPALIVE, &opt, sizeof(opt));
	if (status < 0) {
		System_printf("OscillogrammsWorker: setsockopt SO_KEEPALIVE failed\n");
		fdClose(client_fd);
		return;
	}
	...
	opt = 1;
	status = setsockopt(client_fd, IPPROTO_TCP, TCP_NODELAY, &opt, sizeof(opt));
	if (status < 0) {
		System_printf("OscillogrammsWorker: setsockopt TCP_NODELAY failed\n");
		fdClose(client_fd);
		return;
	}
		...
		switch (cmd) {
			...
			case PACKET_CMD_OSCI: {
				...
				for (Uint16 i = 0; i < arg; i++) {
					...
					packets[i] = *packet_ptr;
					packet_ptr--;
				}

				tcp_send_all(client_fd, &osci_response, sizeof(Osci_Response));
				tcp_send_all(client_fd, packets, osci_response.len);
			} break;

			case PACKET_CMD_FULL: {
				// IVAN: send whole memory
				osci_response.cmd = PACKET_CMD_FULL;
				...
				struct {
					...
					Uint16 after_offset;
				} info_data = { before_offset, after_offset };
				osci_response.len = sizeof(info_data);
