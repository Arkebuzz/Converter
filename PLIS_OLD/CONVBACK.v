...
//altfp_convert CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone III" OPERATION="FLOAT2INT" ROUNDING="TO_NEAREST" WIDTH_DATA=64 WIDTH_EXP_INPUT=11 WIDTH_EXP_OUTPUT=8 WIDTH_INT=32 WIDTH_MAN_INPUT=52 WIDTH_MAN_OUTPUT=23 WIDTH_RESULT=32 clock dataa nan overflow result underflow
...
//altbarrel_shift CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone III" PIPELINE=2 SHIFTDIR="LEFT" SHIFTTYPE="LOGICAL" WIDTH=83 WIDTHDIST=7 aclr clk_en clock data distance result
//VERSION_BEGIN 13.0 cbx_altbarrel_shift 2013:06:12:18:03:43:SJ cbx_mgl 2013:06:12:18:05:10:SJ  VERSION_END

//synthesis_resources = reg 171 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
module  CONVBACK_altbarrel_shift_suf
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	reg	[1:0]	dir_pipe;
	reg	[82:0]	sbit_piper1d;
	reg	[82:0]	sbit_piper2d;
	reg	sel_pipec4r1d;
	reg	sel_pipec5r1d;
	reg	sel_pipec6r1d;
	wire  [7:0]  dir_w;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper1d <= 83'b0;
		else if  (clk_en == 1'b1)   sbit_piper1d <= smux_w[331:249];
	// synopsys translate_off
	initial
		sbit_piper2d = 0;
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper2d <= 83'b0;
		else if  (clk_en == 1'b1)   sbit_piper2d <= smux_w[580:498];
	// synopsys translate_off
	initial
		sel_pipec4r1d = 0;
	// synopsys translate_on
	...
	assign
		...
		direction_w = 1'b0,
		pad_w = {64{1'b0}},
		result = sbit_w[663:581],
		sbit_w = {sbit_piper2d, smux_w[497:332], sbit_piper1d, smux_w[248:0], data},
		sel_w = {sel_pipec6r1d, sel_pipec5r1d, sel_pipec4r1d, distance[3:0]},
		smux_w = {((({83{(sel_w[6] & (~ dir_w[6]))}} & {sbit_w[516:498], pad_w[63:0]}) | ({83{(sel_w[6] & dir_w[6])}} & {pad_w[63:0], sbit_w[580:562]})) | ({83{(~ sel_w[6])}} & sbit_w[580:498])), ((({83{(sel_w[5] & (~ dir_w[5]))}} & {sbit_w[465:415], pad_w[31:0]}) | ({83{(sel_w[5] & dir_w[5])}} & {pad_w[31:0], sbit_w[497:447]})) | ({83{(~ sel_w[5])}} & sbit_w[497:415])), ((({83{(sel_w[4] & (~ dir_w[4]))}} & {sbit_w[398:332], pad_w[15:0]}) | ({83{(sel_w[4] & dir_w[4])}} & {pad_w[15:0], sbit_w[414:348]})) | ({83{(~ sel_w[4])}} & sbit_w[414:332])), ((({83{(sel_w[3] & (~ dir_w[3]))}} & {sbit_w[323:249], pad_w[7:0]}) | ({83{(sel_w[3] & dir_w[3])}} & {pad_w[7:0], sbit_w[331:257]})) | ({83{(~ sel_w[3])}} & sbit_w[331:249])), ((({83{(sel_w[2] & (~ dir_w[2]))}} & {sbit_w[244:166], pad_w[3:0]}) | ({83{(sel_w[2] & dir_w[2])}} & {pad_w[3:0], sbit_w[248:170]})) | ({83{(~ sel_w[2])}} & sbit_w[248:166])), ((({83{(sel_w[1] & (~ dir_w[1]))}} & {sbit_w[163:83], pad_w[1:0]}) | ({83{(sel_w[1] & dir_w[1])}} & {pad_w[1:0], sbit_w[165:85]})) | ({83{(~ sel_w[1])}} & sbit_w[165:83])), ((({83{(sel_w[0] & (~ dir_w[0]))}} & {sbit_w[81:0], pad_w[0]}) | ({83{(sel_w[0] & dir_w[0])}} & {pad_w[0], sbit_w[82:1]})) | ({83{(~ sel_w[0])}} & sbit_w[82:0]))};
endmodule //CONVBACK_altbarrel_shift_suf

//synthesis_resources = lpm_add_sub 5 lpm_compare 4 reg 405 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
module  CONVBACK_altfp_convert_49p
	( 
	clock,
	dataa,
	nan,
	...
	wire  [82:0]   wire_altbarrel_shift6_result;
	...
	wire  [82:0]  barrel_mantissa_input;
	...
	wire  [82:0]  lbarrel_shift_result_w;
	...
	CONVBACK_altbarrel_shift_suf   altbarrel_shift6
	( 
	.aclr(aclr),
	.clk_en(clk_en),
	.clock(clock),
	...
	assign
		...
		clk_en = 1'b1,
		const_bias_value_add_width_res_w = 11'b10000011110,
		...
		lbarrel_shift_w = lbarrel_shift_result_w[82:52],
...
endmodule //CONVBACK_altfp_convert_49p
//VALID FILE
...
module CONVBACK (
	...
	CONVBACK_altfp_convert_49p	CONVBACK_altfp_convert_49p_component (
				.clock (clock),
				.dataa (dataa),
				.overflow (sub_wire0),
				.underflow (sub_wire1),
...
// Retrieval info: GEN_FILE: TYPE_NORMAL CONVBACK.v TRUE FALSE
// Retrieval info: GEN_FILE: TYPE_NORMAL CONVBACK.qip TRUE FALSE
// Retrieval info: GEN_FILE: TYPE_NORMAL CONVBACK.bsf TRUE TRUE
// Retrieval info: GEN_FILE: TYPE_NORMAL CONVBACK_inst.v TRUE TRUE
// Retrieval info: GEN_FILE: TYPE_NORMAL CONVBACK_bb.v TRUE TRUE
// Retrieval info: GEN_FILE: TYPE_NORMAL CONVBACK.inc FALSE TRUE
// Retrieval info: GEN_FILE: TYPE_NORMAL CONVBACK.cmp FALSE TRUE
